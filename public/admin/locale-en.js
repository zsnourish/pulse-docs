/**
 * Full English locale, copied from decap-cms-locales' source (so nothing
 * else silently breaks) with two strings changed: "Contents" -> "Docs" and
 * "New Post" -> "New doc". Terminology unification per request — "doc"
 * everywhere instead of "post"/"content".
 *
 * IMPORTANT LESSON (kept here on purpose): a first attempt called
 * `CMS.registerLocale('en', { app: { header: { content: 'Docs' } }, ... })`
 * with only the two changed keys. Decap's `registerLocale('en', phrases)`
 * REPLACES the built-in English locale rather than merging into it, so every
 * string not explicitly included disappeared, and the UI fell back to
 * showing raw i18n keys like "app.header.workflow" and
 * "collection.sidebar.collections" instead of real text. Registering the
 * complete phrase tree (below) avoids that — anything not deliberately
 * changed stays exactly as Decap's own default.
 * https://github.com/decaporg/decap-cms/blob/main/packages/decap-cms-locales/src/en/index.js
 */
const en = {
  auth: {
    login: 'Login',
  ...
  collection: {
    ...
    defaultFields: {
      author: { label: 'Author' },
      updatedOn: { label: 'Updated On' },
    },
  },
  editor: {
    editorControl: {
      field: { optional: 'optional', widgetLabel: '%{widgetLabel} field' },
    },
    editorControlPane: {
      widget: {
    ...
    editorWidgets: {
      markdown: {
        ...
        richText: 'Rich Text',
        markdown: 'Markdown',
        toggleMode: { rich: 'Toggle to rich text mode', markdown: 'Toggle to markdown mode' },
      },
      image: {
      ...
      datetime: { now: 'Now', clear: 'Clear', setToNow: 'Set %{fieldLabel} to now' },
      list: { add: 'Add %{item}', addType: 'Add %{item}' },
      object: { expand: 'Expand', collapse: 'Collapse' },
    },
  },
  mediaLibrary: {
    mediaLibraryCard: {
  ...
  ui: {
    ...
    settingsDropdown: { logOut: 'Log Out', account: 'Account options dropdown' },
    toast: {
