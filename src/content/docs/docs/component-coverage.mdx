...
Cross-platform build status for Pulse components — Figma design, Vue build, and React build, tracked per platform. This lives in Confluence rather than here, so the team can edit rows without going through GitHub at all.

<p class="tracker-embed-note">
<strong>Heads up:</strong> this embeds the real, published Confluence page (table populated and cross-checked against Figma and Storybook) — but same as the Figma embeds elsewhere on this site, it usually only renders for people already signed into your organisation's Atlassian account, so anonymous visitors are likely to see a login wall instead of the table. Worth checking Confluence's sharing settings if this needs to be visible to people outside the org.
</p>

<div class="ds-embed">
<iframe src="https://nourishcare.atlassian.net/wiki/spaces/Design/pages/1293647876/Component+Tracker" loading="lazy" allowfullscreen></iframe>
<p class="ds-embed-caption">Component Build Tracker — Confluence (Pulse Components v1)</p>
</div>
